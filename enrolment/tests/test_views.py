...
def test_submit_enrolment_logged_out_has_company_redirects(
    ...
    assert response.get('Location') == (
         ...
         '?next=http%3A//testserver/find-a-buyer/register-submit/'
    )
...
def test_landing_page_submit_valid_form_redirects(client):
    ...
    expected_url = '/find-a-buyer/register/company/?company_number=12345678'
    assert response.status_code == 302
...
def test_confirm_company_resets_storage(client, settings):
    ...
    with patch(
        ...
        expected_url = '/find-a-buyer/register/company/?company_number={}'
        assert response.status_code == 302
        assert response.get('Location') == expected_url.format(company_number)
...
def test_submit_enrolment_handles_company_not_active(client, settings):
    ...
    with patch('enrolment.validators.company_active') as mock:
        ...
        response = client.get(
            reverse('register-submit'), {'company_number': '12345678'}
        )
...
def test_enrolment_form_complete_redirects_to_submit_enrolment(sso_request):
    ...
    assert response.status_code == 302
    assert response.get('Location') == (
        '/find-a-buyer/register-submit/?company_number=12345678'
    )
